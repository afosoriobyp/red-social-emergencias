...
import { Report, GRAVITY_META, GRAVITY_LEVELS } from "@/lib/types";
import { useRealtimeReports } from "@/hooks/useRealtimeReports";
import { useCriticalAlert } from "@/hooks/useCriticalAlert";
import ReportCard from "@/components/ReportCard";
import ReportForm from "@/components/ReportForm";
import CategoryFilter from "@/components/CategoryFilter";
import BottomBar, { ViewMode } from "@/components/BottomBar";
import PushManager from "@/components/PushManager";
import { cacheReports, getCachedReports, isOnline, onOnline, getQueuedActions, clearQueue } from "@/lib/idb";
import {
  Bell,
  ChevronDown,
  Search,
...
export default function AppShell({
  ...
  useEffect(() => {
    ...
    setTimeout(() => setIsOffline(!online), 0);
    if (online) {
      cacheReports(initialReports as unknown as Array<{ id: string } & Record<string, unknown>>);
    } else {
      getCachedReports().then((cached) => {
        if (cached.length) setReports(cached as unknown as Report[]);
      });
    }
    const off = onOnline(() => {
      setIsOffline(false);
      cacheReports(reports as unknown as Array<{ id: string } & Record<string, unknown>>);
      flushQueue();
    });
    return () => off();
  }, [initialReports, reports, setReports, flushQueue]);
  ...
  const filtered = useMemo(() => {
    let out = [...reports].sort(
      (a, b) =>
        sortWeight(b) - sortWeight(a) ||
        new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime(),
    );
    if (filter !== "todos") out = out.filter((r) => r.category === filter);
    ...
    return out;
  }, [reports, filter, statusFilter, gravity, query, nearby, nearbyLat, nearbyLng]);
  ...
  function handleUpvote(id: string, upvotes: number) {
    setReports((prev) =>
  ...
  async function openReport() {
    ...
    try {
      ...
      if (data.user?.status === "pendiente") {
        setNotice(
          "Tu cuenta está pendiente de aprobación. Un administrador debe aprobarla para que puedas reportar.",
        );
        return;
      }
      if (data.user?.status === "bloqueado") {
  ...
  return (
    ...
    >
      {isOffline && (
        <div className="fixed top-0 left-0 right-0 z-40 flex items-center justify-center gap-2 px-4 py-1.5 bg-amber-600 text-white text-xs font-medium shadow-lg">
          <WifiOff size={14} /> Modo offline: los cambios se sincronizarán al volver a conectar
        </div>
      )}
      {view === "feed" && (
          ...
          <div className="flex items-center gap-2 px-4 pt-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Buscar reportes, lugares, ayuda…"
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      const res = await fetch(
                        `/api/reports?page=${nextPage}&limit=20`,
                      );
                      const data = await res.json();
      ...
      {view === "map" && (
        <MapView reports={filtered} onLocate={() => undefined} />
      )}
      ...
      {notice && (
        <div className="fixed left-1/2 top-16 z-[1600] w-[calc(100%-2rem)] max-w-md -translate-x-1/2">
          <div className="flex w-full items-center gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 shadow-xl">
            <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-amber-100 text-amber-600">
              <ShieldAlert size={20} />
            </span>
            <p className="flex-1 text-sm font-medium text-amber-800">{notice}</p>
            <button
              onClick={() => setNotice("")}
      ...
      {mode === "memory" && (
        <p className="fixed bottom-20 left-1/2 z-30 -translate-x-1/2 rounded-full bg-amber-500/95 px-3 py-1 text-[11px] font-semibold text-white shadow-md">
          Modo demo · define MONGODB_URI para persistir
        </p>
      )}
...
function sortWeight(r: Report): number {
